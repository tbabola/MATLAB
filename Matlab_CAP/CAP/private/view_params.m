...
if nargin < 1	
    ...
    text(.750,.61,'ISI (msec):','fontsize',10,'color','k','horizontalalignment','right','VerticalAlignment','middle','EraseMode','none');
    ...
    View.parm_txt(14)  = text(.905,.61,num2str(Stimuli.isi), 'fontsize',10,'color',[.1 .1 .6],'horizontalalignment','right','VerticalAlignment','middle','EraseMode','normal','buttondownfcn','view_params(''Stimuli'',14);');
...
elseif strcmp(command_str,'Stimuli')
    if length(get(View.push.edit,'string'))
        ...
        case 14,
            Stimuli.isi = str2num(get(View.push.edit,'string'));
        end
...
elseif strcmp(command_str,'defs')
    set(View.push.update,'Enable','on');
    %GQ modified 9-5-08
    warndlg('Are you sure you want to use default parameters?','Warning',1);
...
%         'automatic',   'no');
    ...
    set(View.parm_txt(14),'String',num2str(Stimuli.isi));
