function [str,cur_line] = struct2str(s,f_units,max_width,max_struct_elem, ident_str,total_ident_len,str,cur_line)
%
...
fnames = fieldnames(s);
fnames_lbl = build_labels(fnames,f_units);
...
for i = 1:length(fnames)
   ...
   if (isstruct(x))
      ...
      for xi = 1:min(length(x),max_struct_elem)
         if (length(f_units) >= i)
            sub_f_units = f_units{i};
         else
            sub_f_units = {};
         end
         [str,cur_line] = struct2str(x(xi),sub_f_units,max_width,max_struct_elem, new_ident_str,new_ident_len,str,cur_line);
         cur_line = cur_line+1;
         str{cur_line} = ident_str;
      end
