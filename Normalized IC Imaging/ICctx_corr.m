...
for i=1:t
    ...
    V1signal(i) = squeeze(mean(Xw(hvcindices)));
end
...
save([dname  name '_corrdfof'], 'corrmat','corrx','lag','ICsignal','ACsignal','V1signal');
...
for i=1:m
    ...
    temp = corr([ICsignal',ACsignal',V1signal'])
...
plot(smooth((V1signal-min(V1signal))/max((V1signal-min(V1signal)))),'k');
set(gcf, 'Renderer', 'Painters');
