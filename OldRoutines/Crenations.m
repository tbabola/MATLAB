function Crenations()
    disp('Crenations');
    direct = 'D:\Bergles Lab Data\RecordingsandImaging\150410\';
    mkdir(direct,'figProcessed');
    %direct = '/Users/Travis/Desktop/141203/A2P2/';
    files = getFiles(direct);
    %files = zero-s(1);

    for i=1:size(files,1)
        %file nameing
        rawFname = files(i).name;
        disp(rawFname);
        %rawFname = '141210_Vid_1_baseline.tif';
        fullFile = strcat(direct,rawFname);
        split = strsplit(rawFname,'.');
        ext = split(2);
        fname = split(1);
        
        %load image from file
        loadedImg = loadImg(fullFile);
        disp('Loaded');
        ...
        %filtImg = filt(thrImg);
        outputFileName = char(strcat(direct,'figProcessed\',fname,'_filt.',ext));
        writeImg(alt2FiltImg, outputFileName);
        if i == 1
            intens = zeros(600,size(files,1)); %holds intensity data
        end
        sq = squeeze(mean(mean(alt2FiltImg,1)));
        intens(1:size(sq,1),i) = sq;
