...
%         indexA = int32(cell2mat(param.idx(param.idx_indexA)));
        indexA = param.idx_indexA; % jbi hack...
    else
...
%         Da_norm = norm(Da,2);
        Da_norm = normest(Da,1e-3); % to speed up approximation (jbi, 2-8-16)
    end
else
    Da_norm = 1;
end
...
while iter_count <= num_iter
    ...
    if param.start_A || do_A
        ...
        if ~param.start_A
            ...
            if param.display
                fprintf([num2str(iter_count) '\t']);
                if iter_count==num_iter || ~mod(iter_count,10), fprintf('\n'); end
                %disp(iter_count);
            end
